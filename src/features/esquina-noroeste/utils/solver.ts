...
export function solveNorthwestCorner(exercise: NorthwestExercise): NorthwestSolution {
  ...
  const allocations: (number | null)[][] = Array.from({length: rows}, () => Array(cols).fill(null));
  const crossedRows = new Set<number>();
  const crossedCols = new Set<number>();
  const frames: NorthwestFrame[] = [];
